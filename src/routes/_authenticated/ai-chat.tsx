...
async function searchProducts(query: string) {
  const { data } = await supabase
    ...
    .or(
      `name_ar.ilike.%${sanitizeSearchTerm(query)}%,gpc_class.ilike.%${sanitizeSearchTerm(query)}%,az_code.ilike.%${sanitizeSearchTerm(query)}%`,
    )
    .limit(5);
  return data ?? [];
}
  ...
  return (
    <>
      ...
      <div className="flex flex-col h-[calc(100vh-160px)] max-w-3xl mx-auto p-4 gap-3">
        ...
        {/* الاقتراحات الذكية */}
        <ChatSmartSuggestions messages={msgs} onSuggestionClick={send} />
